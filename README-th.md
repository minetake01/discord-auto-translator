...
เชื่อมต่อแต่ละช่องทางตามภาษาให้เป็น **กลุ่มแปลภาษา** ทุกข้อความที่โพสต์ในช่องทางหนึ่งจะถูกแปลทันทีโดยโมเดล Chat Completions ที่เข้ากันได้กับ OpenAI และสะท้อนไปยังช่องทางอื่น ๆ ทั้งหมดในกลุ่ม โดยยังคงชื่อและรูปอวาตาร์ของผู้ส่งต้นฉบับไว้ ทำให้แต่ละช่องทางอ่านได้เหมือนการสนทนาธรรมชาติในภาษาของตัวเอง
...
- **ทุกอย่างซิงค์กัน** — ไม่ใช่แค่ข้อความใหม่ แต่การแก้ไข การลบ การตอบกลับ ข้อความที่ส่งต่อ รีแอ็กชัน การปักหมุด เธรด (ช่องทางข้อความ / ฟอรัม / มีเดีย) แท็กฟอรัมที่จับคู่แล้ว และข้อความที่มีแต่ไฟล์แนบ ล้วนถูกสะท้อนไปยังทั้งกลุ่ม
- **ข้อความดูเหมือนส่งมาจากผู้ส่งตัวจริง** — ข้อความที่สะท้อนจะส่งผ่านเว็บฮุกโดยใช้ชื่อและรูปอวาตาร์ของผู้เขียนต้นฉบับ
- **การแปลที่เป็นธรรมชาติ** — the translation model ใช้ชื่อช่องทาง หัวข้อ และประวัติการสนทนาล่าสุดเป็นบริบท นอกจากนี้คำศัพท์เฉพาะของเซิร์ฟเวอร์ยังช่วยกำหนดการแปลที่ต้องการสำหรับชื่อและศัพท์เฉพาะได้
...
- เอนด์พอยต์ Chat Completions ที่เข้ากันได้กับ OpenAI (base URL, API key และ model ID)
...
### 2. ตั้งค่า API ที่เข้ากันได้กับ OpenAI

1. เลือกผู้ให้บริการ Chat Completions ที่เข้ากันได้กับ OpenAI และจด base URL (รวม `/v1` หากผู้ให้บริการใช้คำนำหน้านั้น)
2. สร้าง API key ที่มีสิทธิ์เรียก Chat Completions สำหรับโมเดลที่เลือก
3. ตั้งค่า `OPENAI_BASE_URL`, `OPENAI_API_KEY` และ `OPENAI_MODEL` ใน `.env`

ค่า timeout และขีดจำกัดเอาต์พุตถูกกำหนดในโค้ด (60 วินาทีต่อครั้ง, `max_tokens=4096`) Model ID เป็นการตั้งค่า deployment ในเครื่องที่จำเป็น คุณสามารถปรับ throughput ของโทเค็นต่อเซิร์ฟเวอร์ด้วย `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` ได้ตามต้องการ (ค่าเริ่มต้น `100000`)
...
| `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` | ไม่ | จำกัดโทเค็น translation ต่อเซิร์ฟเวอร์ต่อนาที (ค่าเริ่มต้น: `100000`) |
...
| `/join-channel group:[กลุ่ม] language:[ภาษา] channel:<ช่อง>` | เพิ่มช่องทางเข้ากลุ่ม หากเป็นฟอรัม/สื่อและมีเพียร์ที่มีแท็กในกลุ่ม จะเปิด UI จับคู่แท็ก หากไม่ระบุ `channel` จะใช้ช่องที่รันคำสั่ง |
...
| `/edit-forum-tags group:[กลุ่ม] channel:<ฟอรัม>` | แก้ไขการจับคู่แท็กฟอรัม/สื่อของช่องในกลุ่ม หากไม่ระบุ `channel` จะใช้ช่องที่รันคำสั่ง |
...
- `attribute` แนะนำ "ชื่อบุคคล" "ชื่อสถานที่" "คำแสลง" "ตัวย่อ" และ "คำศัพท์เทคนิค" แต่สามารถป้อนค่าใดก็ได้อย่างอิสระ คุณลักษณะนี้ใช้เป็นบริบทให้ the translation model เข้าใจความหมายของคำศัพท์
...
- ประเภทช่องทางที่รองรับ: ข้อความ ข่าวสาร ฟอรัม และมีเดีย ช่องทั้งหมดในกลุ่มต้องเป็นประเภทเดียวกัน
- การจับคู่แท็กฟอรัม/สื่อตั้งค่าได้หลัง `/join-channel` (เมื่อมีเพียร์ที่มีแท็ก) หรือด้วย `/edit-forum-tags` บันทึกด้วย “(ไม่มีการจับคู่)” จะลบคู่ดังกล่าว
