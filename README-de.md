...
Verknüpfe je einen Channel pro Sprache zu einer **Übersetzungsgruppe**. Jede Nachricht, die in einem Channel gepostet wird, wird sofort von Google Gemini übersetzt und in alle anderen Channels der Gruppe gespiegelt — mit dem Namen und Avatar des Originalsenders — sodass sich jeder Channel wie eine natürliche Unterhaltung in der jeweiligen Sprache liest.
...
- **Natürliche Übersetzungen** — Gemini verwendet Channel-Name, Thema und den jüngsten Gesprächsverlauf als Kontext; ein serverseitiges Glossar erlaubt es, bevorzugte Übersetzungen für Namen und Fachbegriffe festzulegen.
...
- **Effizient und sicher** — Nachrichten ohne übersetzbaren Text (URLs, Erwähnungen, benutzerdefinierte Emojis, Code) werden gespiegelt, ohne die Gemini API aufzurufen; pro Server gelten Token-Ratenlimits; URLs, Erwähnungen und Code-Blöcke sind gegen Prompt-Injection geschützt.
...
### 2. Gemini API-Schlüssel holen

Hole dir einen API-Schlüssel von [Google AI Studio](https://aistudio.google.com/).
...
| `GEMINI_API_KEY` | Ja | Gemini API-Schlüssel |
| `DB_PATH` | Nein | Pfad zur SQLite-Datei (Standard: `./translator.db`) |
| `HTTP_ADDR` | Nein | Adresse des Avatar-Badge-Servers (Standard: `:8080`) |
| `PUBLIC_BASE_URL` | Nein | Basis-URL für das Hinzufügen eines orangefarbenen Ring-Badges zu Avataren |
...
- `attribute` schlägt „Personenname", „Ortsname", „Slang", „Abkürzung" und „Fachbegriff" vor, aber jeder Wert kann frei eingegeben werden. Das Attribut wird als Kontext genutzt, damit Gemini die Bedeutung des Begriffs versteht
...
```powershell
# Erstmalige Einrichtung (Caddy + systemd installieren)
