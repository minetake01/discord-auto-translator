...
언어별로 채널을 하나씩 만들어 **번역 그룹**으로 연결하면, 한 채널에 올라온 메시지가 OpenAI 호환 Chat Completions 모델로 즉시 번역되어 그룹 내 다른 모든 채널로 미러링됩니다. 원래 작성자의 이름과 아바타가 그대로 표시되므로, 각 채널은 해당 언어로 진행되는 자연스러운 대화처럼 보입니다.
...
- **자연스러운 번역** — 번역 모델은 채널 이름, 주제, 최근 대화 기록을 문맥으로 참고합니다. 서버별 용어집으로 인명이나 전문 용어의 번역을 고정할 수도 있습니다.
...
- OpenAI 호환 Chat Completions 엔드포인트(베이스 URL, API 키, 모델 ID)
...
1. OpenAI 호환 Chat Completions 제공자를 선택하고 베이스 URL(`/v1`을 쓰는 경우 포함)을 확인합니다.
2. 선택한 모델로 Chat Completions를 호출할 수 있는 API 키를 만듭니다.
3. `.env`에 `OPENAI_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_MODEL`을 설정합니다.

타임아웃과 출력 한도는 코드에 고정됩니다(시도당 60초, `max_tokens=4096`). 모델 ID는 필수 로컬 배포 설정입니다. 선택적으로 `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`(기본값 `100000`)으로 서버별 토큰 한도를 조정할 수 있습니다.
...
| `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` | 선택 | 서버(길드)별 분당 번역 토큰 상한(기본값: `100000`) |
...
- `attribute`에는 "인명", "지명", "속어", "약어", "전문 용어" 등의 후보가 표시되며 임의의 속성도 자유롭게 입력할 수 있습니다. 지정한 속성은 번역 모델이 용어의 의미를 판단하는 문맥으로 사용됩니다
...
- 지원 채널 유형: 텍스트, 공지, 포럼, 미디어. 그룹 내 모든 채널은 같은 종류여야 합니다.
