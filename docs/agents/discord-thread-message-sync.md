...
| Thread without a source message | `POST /channels/{channel.id}/threads` emits `THREAD_CREATE`. There is no parent message to bind to. | Call `POST /channels/{target_parent}/threads` for non-forum/media targets when the first real thread message arrives (or `SyncThreadCreate` is used without defer). | `source thread id -> target thread id`. | Never call create-thread-from-message for this pattern. Gateway `THREAD_CREATE` alone does not translate or create. |
| Normal message inside an existing thread | `MESSAGE_CREATE` has `channel_id` equal to the source or target thread ID. | Execute the target parent webhook with `thread_id={peer_thread_id}` and `wait=true`. | `source thread message -> target thread message`. | Webhook edit/delete/get also need `thread_id` for webhook messages inside threads. If the thread is already stored as a source or a target, do not create another peer thread. |
| Forum/media new post | `POST /channels/{forum_or_media}/threads` creates the thread and the first message together. The returned channel includes the created post/thread information. | For forum/media targets, call the same thread endpoint with translated title, translated first message content, and mapped `applied_tags`. Gateway `THREAD_CREATE` without the initial payload defers without translating. | `source post thread -> target post thread` and `source first message -> target first message`. | Map source tag IDs through `forum_tag_maps`. Omit unmapped tags. Fail the target create when the destination has `REQUIRE_TAG` and the mapped set is empty. Translation groups must be same channel type, so forum/media posts only target forum/media peers. |
...
| Forum tag change | `THREAD_UPDATE` may change `applied_tags`. | `PATCH /channels/{target_thread_id}` with mapped `applied_tags`. | Existing thread links only. | Skip when the mapped set already matches the target. |
...
- `THREAD_CREATE` from the gateway uses `SyncThreadCreateFromGateway`, which defers without translating when the translated parent message link is not available yet, or when a forum/media post has no initial payload yet.
...
- `ensureThreadSynced` creates peer threads only when the thread ID is not already stored as a source or a target. Messages in a mirrored thread use `ThreadTargets` reverse mapping and do not create a second family of threads.
...
- Forum/media creates pass mapped `AppliedTags` from `forum_tag_maps`. Tag mappings are configured via `/join-channel` (when peers exist) and `/edit-forum-tags`.
...
- `THREAD_CREATE -> THREAD_STARTER_MESSAGE` when the parent message link appears later (no translation on the deferred gateway event).
...
- A message posted in an already-mirrored target thread mirroring back without creating a reverse thread.
