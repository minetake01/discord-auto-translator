...
翻訳は `us-west-2` の Amazon Bedrock Mantle Responses API で `google.gemma-4-26b-a4b` を呼び出します。モデル ID、リージョン、エンドポイントはコード固定で、環境変数では変更できません。Gemma 4 は `bedrock-runtime`、Invoke、Converse に対応しないため、`bedrock-mantle` へSigV4署名したHTTPリクエストを直接送ります。
...
**環境変数（必須）:** `AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY`。専用 IAM ユーザーにはMantleのデフォルトProjectに対する `bedrock-mantle:CreateInference` を許可します。AWS公式の基本ポリシーでは `GetProject`、`ListProjects`、`ListTagsForResources` も併記されます。任意: `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`（デフォルト `100000`）。
...
| `.env`（デフォルト） | `DISCORD_TOKEN`・`AWS_ACCESS_KEY_ID`・`AWS_SECRET_ACCESS_KEY`・`TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`・`PUBLIC_BASE_URL` 等 |
