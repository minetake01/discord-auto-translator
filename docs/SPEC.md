...
**権限**: デフォルトではサーバー管理者のみ実行可能です。環境変数 `ADMIN_ROLE_IDS` にカンマ区切りでロール ID を指定すると、そのロールを持つメンバーも実行できます。Discord API 側で `default_member_permissions` を管理者に設定し、指定ロールにはコマンドごとに allow を付与します。実行時にもハンドラで再チェックします。
...
投稿時は送信者の表示名とアバター画像をウェブフックのユーザー名・アバターに設定します。TTS フラグはミラー先にも引き継ぎます。ステッカーは Discord CDN から画像として取得し、添付ファイルとして転送します（Lottie は PNG ラスタ化 URL を試行し、取得不能な場合はそのステッカーのみスキップ）。
...
返信元メッセージが翻訳グループに属する場合、対象言語に翻訳した引用スニペット（最大 20 文字）を先頭に付与します。
...
返信元著者へのメンションが含まれている場合は `<@ユーザーID>` も追加されます。

引用スニペットの本文はゲートウェイの `referenced_message` を優先し、`message_links` が無い場合でも返信元チャンネル上のメッセージ URL へリンクします。ミラー先チャンネル内の対応メッセージが DB にある場合は、その URL を使用します。
### 3.4 ピン留め同期
