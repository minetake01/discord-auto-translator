...
あるチャンネルにメッセージが投稿されると、ボットがそのメッセージを Cloudflare Workers AI の `@cf/google/gemma-4-26b-a4b-it`（AI Gateway 経由）で翻訳し、同じ「翻訳グループ」に属する他のチャンネルへウェブフックで投稿します。投稿者の名前とアバターを偽装できるため、ユーザーには自分の言語でネイティブに会話しているように見えます。
...
| 翻訳エンジン | Cloudflare Workers AI `@cf/google/gemma-4-26b-a4b-it`（AI Gateway 経由、`internal/translatorbot/cloudflare_translator.go`） |
...
翻訳に失敗した場合、またはギルド単位の翻訳トークンレート制限（`TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`、デフォルト `100000`）を超過した場合はミラーリングせず、投稿元チャンネルへ通知を投稿します（fail-closed）。通知文言は投稿元チャンネルに登録された言語で表示されます。
...
- 属性は選別された用語の `<attribute>` としてシステム指示へ渡し、Gemma 4 が用語の意味・役割を判断する文脈として使用する
...
### 3.13 翻訳 API（Cloudflare AI Gateway）

翻訳は常に設定済み AI Gateway（`CLOUDFLARE_AI_GATEWAY_ID`）経由で `@cf/google/gemma-4-26b-a4b-it` を呼び出します。モデル ID はコード固定で、環境変数では変更できません。

| 項目 | 値 |
|---|---|
| エンドポイント | `POST /accounts/{account_id}/ai/v1/chat/completions` |
| HTTP タイムアウト | 10 秒 |
| temperature | 0.2 |
| max_tokens | 16384（固定） |
| stream | false |
| reasoning / thinking | 送らない（プロバイダーデフォルト） |
| 出力形式 | strict JSON schema（複数言語 `translations` 配列） |

**環境変数（必須）:** `CLOUDFLARE_ACCOUNT_ID`、`CLOUDFLARE_API_TOKEN`（デプロイ環境ごとに 1 トークン、スコープは運用者が決定）、`CLOUDFLARE_AI_GATEWAY_ID`。任意: `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`（デフォルト `100000`）。

**Gateway 契約:**

- キャッシュ有効。ボットは完全なリクエストボディを送り、cache bypass ヘッダーやカスタム cache key は送らない
- ログはメタデータのみ。`cf-aig-collect-log-payload: false`、`cf-aig-metadata` は `guild_id` と `message_id` のみ
- retry、fallback、動的ルーティング、DLP、guardrails は使用しない（Gateway 側でも無効のまま）
- プロンプト・応答・API トークンをログに出さない
- デプロイと環境選択は利用者側の責任
- 障害・レート制限超過時は fail-closed（ミラーリングしない）

beta 提供のため、ライブ A/B テストや自動品質ゲートは実施しない。
...
```sh
cp .env.example .env
# .env を編集して DISCORD_TOKEN と Cloudflare 設定を入力
...
| `.env`（デフォルト） | `DISCORD_TOKEN`・`CLOUDFLARE_ACCOUNT_ID`・`CLOUDFLARE_API_TOKEN`・`CLOUDFLARE_AI_GATEWAY_ID`・`TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`・`PUBLIC_BASE_URL` 等 |
