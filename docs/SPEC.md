...
返信元が `message_links` に存在する場合、引用スニペットは保存済みの原文スナップショットから生成します。これにより、翻訳済みメッセージに含まれる表示用の引用行を再翻訳しません。対応が無い場合はゲートウェイの `referenced_message` を使用し、返信元チャンネル上のメッセージ URL へリンクします。ミラー先チャンネル内の対応メッセージが DB にある場合は、その URL を使用します。
### 3.4 ピン留め同期
