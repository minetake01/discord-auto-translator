...
├── cloudflare_translator.go # Cloudflare AI Gateway 経由の翻訳 API 呼び出し
...
### Cloudflare モデルと AI Gateway

`cloudflare_translator.go` でモデルとリクエストパラメータが定義されています：

```go
const cloudflareModel = "@cf/google/gemma-4-26b-a4b-it"
// HTTP client timeout: 10s
// temperature: 0.2, max_tokens: 16384
// reasoning/thinking parameters omitted (provider default)
```

翻訳リクエストは常に AI Gateway 経由（`cf-aig-gateway-id` ヘッダー）。キャッシュは有効、payload ログは無効（`cf-aig-collect-log-payload: false`）、metadata は `guild_id` と `message_id` のみ。cache bypass / custom cache key は送らない。retry・fallback・動的ルーティング・DLP・guardrails は無効。

モデル ID は env では変更できません。`CLOUDFLARE_API_TOKEN` はデプロイ環境ごとに 1 つ。障害時は fail-closed（翻訳を送らず、投稿元チャンネルへローカライズ通知）。beta のためライブ A/B や品質ゲートは実施しない。
...
- Gemma 4 の temperature は `0.2`（低め）に設定して一貫性を優先しています。`max_tokens` は `16384` 固定です。
...
- 実際の Cloudflare Workers AI レスポンス
...
| `CLOUDFLARE_ACCOUNT_ID` | 必須 | — | Cloudflare アカウント ID |
| `CLOUDFLARE_API_TOKEN` | 必須 | — | デプロイ環境ごとに 1 つ（スコープは運用者が決定） |
| `CLOUDFLARE_AI_GATEWAY_ID` | 必須 | — | AI Gateway ID（環境ごとに切り替え可能） |
| `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` | 任意 | `100000` | ギルドごとの Gemma 4 トークン上限/分 |
