...
Liên kết một kênh mỗi ngôn ngữ thành một **nhóm dịch thuật**. Mỗi tin nhắn được đăng trên một kênh sẽ được `@cf/google/gemma-4-26b-a4b-it` qua Cloudflare AI Gateway dịch ngay lập tức và phản chiếu đến tất cả các kênh khác trong nhóm — giữ nguyên tên và ảnh đại diện của người gửi gốc — để mỗi kênh đọc như một cuộc trò chuyện tự nhiên bằng ngôn ngữ của mình.
...
- **Dịch thuật tự nhiên** — Gemma 4 sử dụng tên kênh, chủ đề và lịch sử cuộc trò chuyện gần đây làm ngữ cảnh; bảng thuật ngữ theo từng máy chủ cho phép cố định các bản dịch ưa thích cho tên riêng và thuật ngữ chuyên ngành.
...
- Một token API Cloudflare cho mỗi môi trường triển khai
...
### 2. Cấu hình Cloudflare Workers AI và AI Gateway

1. Tạo **một** token API Cloudflare cho mỗi môi trường triển khai; phạm vi do người vận hành quyết định.
2. Tạo [AI Gateway](https://developers.cloudflare.com/ai-gateway/get-started/) cho tài khoản đó và ghi lại ID (`CLOUDFLARE_AI_GATEWAY_ID`).
3. Trong bảng điều khiển Gateway, **bật cache** và **tắt ghi log payload** (chỉ metadata).
4. Giữ retry, fallback, định tuyến động, DLP và guardrails **tắt** — bot không dùng chúng.

Đặt `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN` và `CLOUDFLARE_AI_GATEWAY_ID` trong `.env`. Tùy chọn điều chỉnh throughput bằng `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` (mặc định `100000`).
...
| `CLOUDFLARE_ACCOUNT_ID` | Có | ID tài khoản Cloudflare cho Workers AI / AI Gateway |
| `CLOUDFLARE_API_TOKEN` | Có | Một token API cho mỗi môi trường triển khai (phạm vi do người vận hành quyết định) |
| `CLOUDFLARE_AI_GATEWAY_ID` | Có | ID AI Gateway; có thể khác nhau theo môi trường |
...
| `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` | Không | Giới hạn token Gemma 4 mỗi máy chủ mỗi phút (mặc định: `100000`) |
...
### Hợp đồng vận hành Cloudflare AI Gateway

Dịch thuật dùng `@cf/google/gemma-4-26b-a4b-it` qua [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/) đã cấu hình. ID mô hình cố định trong mã và không thể đổi bằng biến môi trường.

Bot luôn định tuyến yêu cầu dịch qua `CLOUDFLARE_AI_GATEWAY_ID`. Cấu hình **một** token API Cloudflare (`CLOUDFLARE_API_TOKEN`) cho mỗi môi trường triển khai qua biến môi trường; phạm vi do người vận hành quyết định.

Tham số yêu cầu cố định: chat completions không streaming, timeout HTTP **10 giây**, **temperature 0.2**, **max_tokens 16384**, JSON schema strict đa ngôn ngữ. Bỏ qua tham số reasoning/thinking (mặc định nhà cung cấp).

**Gateway (duy trì trong bảng điều khiển):**

- **Cache** — bật; bot gửi toàn bộ request body, không gửi header bypass cache hay custom cache key
- **Ghi log** — chỉ metadata; tắt payload logging; bot gửi `cf-aig-collect-log-payload: false` và `cf-aig-metadata` chỉ với `guild_id` và `message_id`
- **Tính năng tắt** — không dùng retry, fallback, định tuyến động, DLP, guardrails

Bot không ghi log prompt, phản hồi hay token API. Triển khai và chọn môi trường (ID Gateway / tài khoản) do người dùng chịu trách nhiệm. Lỗi dịch và vượt rate limit: **fail-closed** — không phản chiếu tin nhắn; kênh nguồn nhận thông báo bản địa hóa.

Vì Cloudflare Workers AI cho mô hình này đang beta, lần chuyển đổi này không chạy thử A/B trực tiếp hay quality gate tự động.
...
- `attribute` gợi ý "tên người", "tên địa điểm", "tiếng lóng", "từ viết tắt" và "thuật ngữ kỹ thuật", nhưng có thể nhập bất kỳ giá trị nào tùy ý. Thuộc tính được sử dụng làm ngữ cảnh để Gemma 4 hiểu ý nghĩa của thuật ngữ
