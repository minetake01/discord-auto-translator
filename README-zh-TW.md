...
在 `your-aws-bedrock-region` 的 Amazon Bedrock 中啟用 `google.gemma-4-26b-a4b`。建立僅對該模型擁有 `bedrock-mantle:CreateInference` 權限的 IAM 使用者，並在 `.env` 設定 `AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY`、`AWS_BEDROCK_REGION` 與 `AWS_BEDROCK_PROJECT_ID`。模型、逾時與輸出上限固定於程式碼中；區域與 Project ID 是必要的本機部署設定。
