...
為每種語言準備一個頻道，並將它們連結為一個**翻譯群組**。當某個頻道有新訊息時，相容 OpenAI 的 Chat Completions 模型會立即翻譯並鏡像到群組內所有其他頻道 — 保留原發送者的名稱與頭像 — 讓每個頻道讀起來都像是以該語言進行的自然對話。
...
- **自然的翻譯** — 翻譯模型會參考頻道名稱、主題與最近的對話紀錄作為上下文；每個伺服器還可設定詞彙表，為人名與專業術語指定固定譯法。
...
- 相容 OpenAI 的 Chat Completions 端點（基礎 URL、API 金鑰與模型 ID）
...
1. 選擇任意相容 OpenAI 的 Chat Completions 提供者，並確認其基礎 URL（若提供者使用 `/v1` 前綴則一併包含）。
2. 建立可呼叫所選模型 Chat Completions 的 API 金鑰。
3. 在 `.env` 設定 `OPENAI_BASE_URL`、`OPENAI_API_KEY` 與 `OPENAI_MODEL`。

逾時與輸出上限固定於程式碼中（每次嘗試 60 秒，`max_tokens=4096`）。模型 ID 是必要的本機部署設定。可選地使用 `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN`（預設 `100000`）調整每個伺服器的權杖吞吐。
...
| `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` | 選用 | 每個伺服器每分鐘的翻譯權杖上限（預設: `100000`） |
...
- 每個伺服器最多可註冊 50 筆詞彙
- `attribute` 會提示「人名」「地名」「俚語」「縮寫」「專業術語」等候選，也可自由輸入任意屬性。指定的屬性將作為翻譯模型判斷詞彙含義的上下文
