...
Link one channel per language into a **translation group**. Every message posted in one channel is instantly translated by `@cf/google/gemma-4-26b-a4b-it` via Cloudflare AI Gateway and mirrored to all the other channels in the group — with the original sender's name and avatar — so each channel reads like a normal conversation in its own language.
...
- **Natural translations** — Gemma 4 sees the channel name, topic, and recent conversation history as context, and a per-server glossary lets you enforce preferred translations for names and jargon.
...
- One Cloudflare API token per deployment environment
- One Cloudflare AI Gateway ID
...
### 2. Configure Cloudflare Workers AI and AI Gateway

1. Create **one** Cloudflare API token per deployment environment, scoped by the operator.
2. Create an [AI Gateway](https://developers.cloudflare.com/ai-gateway/get-started/) for that account and note its ID (`CLOUDFLARE_AI_GATEWAY_ID`).
3. In the Gateway dashboard, **enable cache** and **disable payload logging** (metadata-only logs).
4. Leave retry, fallback, dynamic routing, DLP, and guardrails **disabled** — the bot does not use them.

Set `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, and `CLOUDFLARE_AI_GATEWAY_ID` in `.env`. Optionally tune per-guild token throughput with `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` (default `100000`).
...
| `CLOUDFLARE_ACCOUNT_ID` | Yes | Cloudflare account ID for Workers AI and AI Gateway |
| `CLOUDFLARE_API_TOKEN` | Yes | One API token per deployment environment (scoped by the operator) |
| `CLOUDFLARE_AI_GATEWAY_ID` | Yes | AI Gateway ID; use a different value per environment when needed |
...
| `TRANSLATION_RATE_LIMIT_TOKENS_PER_MIN` | No | Per-guild Gemma 4 token limit per minute (default: `100000`) |
...
### Cloudflare AI Gateway operational contract

Translation uses `@cf/google/gemma-4-26b-a4b-it` via the configured [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/). The model ID is fixed in code and cannot be changed through environment variables.

The bot always routes translation requests through `CLOUDFLARE_AI_GATEWAY_ID`. Configure **one** Cloudflare API token (`CLOUDFLARE_API_TOKEN`) per deployment environment via environment variable, scoped by the operator.

Fixed request parameters: non-streaming chat completions, **10 s** HTTP timeout, **temperature 0.2**, **max_tokens 16384**, strict JSON schema for multi-language output. Reasoning/thinking parameters are omitted (provider default).

**Gateway (must remain configured on your Gateway):**

- **Cache** — enabled; the bot sends the full request body and does not send a cache bypass header or custom cache key
- **Logging** — metadata only; Gateway payload logging disabled; the bot sends `cf-aig-collect-log-payload: false` and `cf-aig-metadata` with only `guild_id` and `message_id`
- **Disabled features** — retry, fallback, dynamic routing, DLP, and guardrails are not used

The bot does not log prompts, responses, or the API token. Deployment and environment selection (which Gateway ID / account to use) are user-owned. Translation failures and rate-limit breaches are **fail-closed** — the message is not mirrored and the source channel receives a localized notification.

Because the Cloudflare Workers AI offering for this model is beta, this migration does not run a live A/B test or automated quality gate.
...
- `attribute` suggests "person name", "place name", "slang", "abbreviation", and "technical term", but any value can be entered. The attribute is used as context for Gemma 4 to understand the term's meaning
