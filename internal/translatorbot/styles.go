...
var stylePresetInstructions = map[string]string{
	...
	"casual":   nativePhrasingInstruction + ` Default to relaxed, everyday conversational language, like friends chatting.`,
	...
	"gaming":   nativePhrasingInstruction + ` Default to the casual voice of online gaming communities, with gaming slang and abbreviations natural to the target language (e.g. gg, nerf, buff). Keep game titles, character names, and technical game terms recognizable to players.`,
	"friendly": nativePhrasingInstruction + ` Default to warm, friendly, and approachable language, like talking to a good friend.`,
	"netslang": nativePhrasingInstruction + ` Default to the voice of anonymous message boards in the target language (e.g. 2ch/5ch-style for Japanese, imageboard/Reddit-style for English): net slang, abbreviations, and short low-formality sentences; dropping subjects and particles is fine where natural.`,
	"tweet":    nativePhrasingInstruction + ` Default to casual social media phrasing (a tweet): short, punchy, and colloquial. Do not add hashtags, emoji, or mentions that are not in the source text.`,
}

func StylePresetChoices() []*discordgo.ApplicationCommandOptionChoice {
	return []*discordgo.ApplicationCommandOptionChoice{
		{Name: "default (自然なチャット)", Value: StylePresetDefault},
...
func ResolveStyleInstructions(preset, custom string) string {
	...
	preset = strings.TrimSpace(preset)
	if preset == "" || preset == StylePresetDefault {
		return nativePhrasingInstruction
	}
