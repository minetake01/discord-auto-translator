...
const (
	...
	uiKeyPollEnded              uiKey = "pollEnded"
	uiKeyPollResultVictor       uiKey = "pollResultVictor"
	uiKeyPollResultNoWinner     uiKey = "pollResultNoWinner"

	uiKeyRateLimitNotice           uiKey = "rateLimitNotice"
	uiKeyTranslationFailedNotice   uiKey = "translationFailedNotice"
	uiKeyProviderMayContinueNotice uiKey = "providerMayContinueNotice"
	...
	uiKeyChannelTypeMismatch uiKey = "channelTypeMismatch"
...
var uiStrings = map[string]map[uiKey]string{
	"en": {
		...
		uiKeyPollVote:               "Vote",
		uiKeyPollEnded:              "A poll has ended.",
		uiKeyPollResultVictor:       "Result: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "There was no winner.",
		...
		uiKeyProviderMayContinueNotice: "Some messages may go untranslated for a while because of a problem with the external translation service.",
		...
		uiKeyChannelTypeMismatch: "Group `%[1]s` has channels of a different type. All channels in a group must be the same channel type.",
	...
	"ja": {
		...
		uiKeyPollVote:               "投票する",
		uiKeyPollEnded:              "投票が終了しました。",
		uiKeyPollResultVictor:       "結果: %[1]s（%[2]s%%）",
		uiKeyPollResultNoWinner:     "勝者はいませんでした。",
		...
		uiKeyProviderMayContinueNotice: "外部の翻訳サービス側の問題のため、しばらく翻訳漏れが生じる可能性があります。",
		...
		uiKeyChannelTypeMismatch: "グループ `%[1]s` には別の種類のチャンネルがあります。グループ内のチャンネルはすべて同じ種類である必要があります。",
	...
	"zh-CN": {
		...
		uiKeyPollVote:               "去投票",
		uiKeyPollEnded:              "投票已结束。",
		uiKeyPollResultVictor:       "结果：%[1]s（%[2]s%%）",
		uiKeyPollResultNoWinner:     "没有胜者。",

		uiKeyRateLimitNotice:           "已达到翻译速率限制，此消息未被翻译。在限制解除之前，一段时间内消息可能无法翻译。",
		uiKeyTranslationFailedNotice:   "翻译失败，此消息未被镜像。",
		uiKeyProviderMayContinueNotice: "由于外部翻译服务出现问题，一段时间内可能出现漏译。",
		...
		uiKeyChannelTypeMismatch: "组 `%[1]s` 中已有不同类型的频道。同一组内的所有频道必须是相同类型。",
	...
	"zh-TW": {
		...
		uiKeyPollVote:               "去投票",
		uiKeyPollEnded:              "投票已結束。",
		uiKeyPollResultVictor:       "結果：%[1]s（%[2]s%%）",
		uiKeyPollResultNoWinner:     "沒有勝者。",

		uiKeyRateLimitNotice:           "已達到翻譯速率限制，此訊息未被翻譯。在限制解除之前，一段時間內訊息可能無法翻譯。",
		uiKeyTranslationFailedNotice:   "翻譯失敗，此訊息未被鏡像。",
		uiKeyProviderMayContinueNotice: "由於外部翻譯服務出現問題，一段時間內可能出現漏譯。",
		...
		uiKeyChannelTypeMismatch: "群組 `%[1]s` 中已有不同類型的頻道。同一群組內的所有頻道必須是相同類型。",
	...
	"ko": {
		...
		uiKeyPollVote:               "투표하기",
		uiKeyPollEnded:              "투표가 종료되었습니다.",
		uiKeyPollResultVictor:       "결과: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "승자가 없었습니다.",
		...
		uiKeyProviderMayContinueNotice: "외부 번역 서비스 측 문제로, 한동안 번역이 누락될 수 있습니다.",
		...
		uiKeyChannelTypeMismatch: "그룹 `%[1]s`에는 다른 종류의 채널이 있습니다. 그룹 내 모든 채널은 같은 종류여야 합니다.",
	...
	"fr": {
		...
		uiKeyPollVote:               "Voter",
		uiKeyPollEnded:              "Un sondage est terminé.",
		uiKeyPollResultVictor:       "Résultat : %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "Il n’y a pas eu de gagnant.",
		...
		uiKeyProviderMayContinueNotice: "Certains messages pourraient ne pas être traduits pendant un moment en raison d'un problème du service de traduction externe.",
		...
		uiKeyChannelTypeMismatch: "Le groupe `%[1]s` contient des salons d'un type différent. Tous les salons d'un groupe doivent être du même type.",
	...
	"de": {
		...
		uiKeyPollEnded:              "Eine Umfrage ist beendet.",
		uiKeyPollResultVictor:       "Ergebnis: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "Es gab keinen Gewinner.",
		...
		uiKeyProviderMayContinueNotice: "Wegen eines Problems mit dem externen Übersetzungsdienst können Nachrichten vorübergehend unübersetzt bleiben.",
		...
		uiKeyChannelTypeMismatch: "Die Gruppe `%[1]s` enthält Kanäle eines anderen Typs. Alle Kanäle in einer Gruppe müssen denselben Typ haben.",
	...
	"es": {
		...
		uiKeyPollVote:               "Votar",
		uiKeyPollEnded:              "La encuesta ha terminado.",
		uiKeyPollResultVictor:       "Resultado: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "No hubo ganador.",
		...
		uiKeyProviderMayContinueNotice: "Debido a un problema con el servicio de traducción externo, es posible que algunos mensajes no se traduzcan durante un tiempo.",
		...
		uiKeyChannelTypeMismatch: "El grupo `%[1]s` tiene canales de un tipo diferente. Todos los canales de un grupo deben ser del mismo tipo.",
	...
	"pt-BR": {
		...
		uiKeyPollVote:               "Votar",
		uiKeyPollEnded:              "A enquete terminou.",
		uiKeyPollResultVictor:       "Resultado: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "Não houve vencedor.",
		...
		uiKeyProviderMayContinueNotice: "Devido a um problema no serviço de tradução externo, algumas mensagens podem ficar sem tradução por um tempo.",
		...
		uiKeyChannelTypeMismatch: "O grupo `%[1]s` tem canais de um tipo diferente. Todos os canais de um grupo devem ser do mesmo tipo.",
	...
	"it": {
		...
		uiKeyPollVote:               "Vota",
		uiKeyPollEnded:              "Il sondaggio è terminato.",
		uiKeyPollResultVictor:       "Risultato: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "Non c’è stato un vincitore.",
		...
		uiKeyProviderMayContinueNotice: "A causa di un problema del servizio di traduzione esterno, alcuni messaggi potrebbero non essere tradotti per un po'.",
		...
		uiKeyChannelTypeMismatch: "Il gruppo `%[1]s` ha canali di un tipo diverso. Tutti i canali di un gruppo devono essere dello stesso tipo.",
	...
	"id": {
		...
		uiKeyPollEnded:              "Polling telah berakhir.",
		uiKeyPollResultVictor:       "Hasil: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "Tidak ada pemenang.",
		...
		uiKeyProviderMayContinueNotice: "Karena masalah pada layanan terjemahan eksternal, beberapa pesan mungkin tidak diterjemahkan untuk sementara.",
		...
		uiKeyChannelTypeMismatch: "Grup `%[1]s` memiliki channel dengan jenis yang berbeda. Semua channel dalam satu grup harus berjenis sama.",
	...
	"th": {
		...
		uiKeyPollVote:               "โหวต",
		uiKeyPollEnded:              "โพลสิ้นสุดแล้ว",
		uiKeyPollResultVictor:       "ผลลัพธ์: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "ไม่มีผู้ชนะ",
		...
		uiKeyProviderMayContinueNotice: "เนื่องจากปัญหาที่บริการแปลภายนอก ข้อความบางรายการอาจไม่ถูกแปลสักพัก",
		...
		uiKeyChannelTypeMismatch: "กลุ่ม `%[1]s` มีช่องประเภทอื่นอยู่แล้ว ช่องทั้งหมดในกลุ่มต้องเป็นประเภทเดียวกัน",
	...
	"vi": {
		...
		uiKeyPollEnded:              "Cuộc bình chọn đã kết thúc.",
		uiKeyPollResultVictor:       "Kết quả: %[1]s (%[2]s%%)",
		uiKeyPollResultNoWinner:     "Không có người chiến thắng.",
		...
		uiKeyProviderMayContinueNotice: "Do sự cố của dịch vụ dịch thuật bên ngoài, một số tin nhắn có thể không được dịch trong một thời gian.",
		...
		uiKeyChannelTypeMismatch: "Nhóm `%[1]s` đã có kênh thuộc loại khác. Tất cả kênh trong một nhóm phải cùng loại.",
